...
flags.DEFINE_integer(
    "shard_size", 200, "Maximum number of trajectories per shard")
...
def main(_):
    ...
    paths = glob.glob(os.path.join(
        FLAGS.input_path, *("*" * (FLAGS.depth - 1))))

    # get trajecotry paths in parallel
    with Pool(FLAGS.num_workers) as p:
        train_paths, val_paths = zip(
            *p.map(
                partial(get_traj_paths,
                        train_proportion=FLAGS.train_proportion), paths
            )
        )
    ...
    val_shards = np.array_split(val_paths, np.ceil(
        len(val_paths) / FLAGS.shard_size))
